...
void displayIDInfo()
{
	...
	cout << "Lab:     Tue 10am and Tue, 1pm" << endl;
	cout << "Program: #3, 0/1 Guessing" << endl;
	cout << endl;
}//end displayIDInfo()
...
int main()
{
	...
	while (true)
	{
		// Get computerforecast with reseting.
		if (resetForcast == true)		
		{
			computerForcast = resetGetForcast(lastThreeMoves, moveHistory, index);
		}
		// Get computerforecast without reseting.
		else if (resetForcast == false)		
		{
			computerForcast = getForcast(lastThreeMoves, moveCount, moveHistory, index);
		}
		// Displays the history table when the switch of toggle is on.
		if (toggleDisp)		
		{
		...
		if (isalpha(userInput))
		{
			...
			// Reset the move history table.
			if (userInput == 'R')		
			{
				resetForcast = true;

				// Reset the history table.
				resetHistoryTable(moveHistory, lastThreeMoves);		
				moveCount = 0;
				updateHistoryTable(decUserInput, moveHistory, index);
				convertbinaryToDecimal(lastThreeMoves, index);
				board[pointIndicater] = ' ';

				//Reseting the point indicator.
				pointIndicater = 10;		
			}
		}
		else
		{
			// Converts the char userinput to int userinput.
			decUserInput = userInput;
			if (decUserInput == 48 || decUserInput == 49)
			{
				decUserInput = decUserInput - 48;
			}

			// Updates history table without reset command.
			if (moveCount > 3 && resetForcast == false)		
			{
				updateHistoryTable(decUserInput, moveHistory, index);
			}
			// Update history table and last three moves by reset command given.
			else if (resetForcast == true)	
			{
				updateLastThreeMoves(lastThreeMoves, decUserInput);
				updateHistoryTable(decUserInput, moveHistory, index);
			}
			// Updates the last three moves without reset command.
			if (resetForcast == false)		
			{
